...
class MetadataViewGenerator:
    ...
    def create_list_view(model, metadata_class):
        ...
        def list_view(request):
            ...
            # Extract app_name from request path for URL generation
            # Path format: /app/{app_name}/m/{model}/list
            import re
            ...
            if search_query and hasattr(metadata_class, 'list_display'):
                ...
                for field_name in metadata_class.list_display:
                    # Only search text-compatible fields
                    try:
                        field = model._meta.get_field(field_name)
                        field_type = field.get_internal_type()
                        # Support CharField, TextField, and similar text fields
                        if field_type in ('CharField', 'TextField', 'EmailField', 'URLField'):
                            q_objects |= Q(**{f"{field_name}__icontains": search_query})
                    except Exception:
                        # Field not found or not a model field (e.g., property), skip
                        pass
                if q_objects:
            ...
            # Prepare data for the template (only current page)
            items_data = []
    ...
    def create_detail_view(model, metadata_class):
        ...
        def detail_view(request, **kwargs):
            ...
            if hasattr(metadata_class, 'form') and metadata_class.form:
                ...
                # Auto-generate form config from fieldsets when no explicit form is defined
                # This mirrors the fallback logic in create_create_view for consistency
                from django.forms import modelform_factory

                # Extract field names from fieldsets
                fieldset_fields = []
                for fieldset_name, fieldset_config in metadata_class.fieldsets:
                    fieldset_fields.extend(fieldset_config.get('fields', []))

                # Get valid model field names (exclude auto-generated and system fields)
                ...
                if valid_fields:
                    # Dynamically create a ModelForm with these fields
        ...
        def create_view(request):
            ...
            if request.method == 'GET':
                ...
                if form_class:
                    ...
                    # No form class - try to generate fields from fieldsets
                    form_fields = []
                    ...
                    if hasattr(metadata_class, 'fieldsets') and metadata_class.fieldsets:
                        ...
                        if valid_fields:
                            ...
                            # Use existing serialize function to get field configs
                            form_data = serialize_form_for_react(DynamicForm, user=request.user, model_name=model_name_lower)

                            # Convert form_data dict to form_fields list format
                            # Preserve field order from fieldsets
                            widget_to_type = {
                                'TextInput': 'text',
                                'Textarea': 'textarea',
                                'Select': 'select',
                    ...
                    # Fallback to default fields if no fieldsets or no valid fields found
                    if not form_fields:
                        form_fields = [
            ...
            if form_class:
                ...
                # Set owner if the model has an owner field
                # Uses Django's _meta.get_field() for reliable detection of inherited fields
                try:
