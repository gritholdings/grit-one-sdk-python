...
if hasattr(home_views, 'OnboardingView'):
    urlpatterns += [
        path('onboarding/', RedirectView.as_view(pattern_name='onboarding', permanent=False),
             {'step': 1}, name='onboarding_default'),
        path('onboarding/<int:step>/', home_views.OnboardingView.as_view(), name='onboarding'),
    ]
else:
    urlpatterns += [
        path('onboarding/', RedirectView.as_view(url='/', permanent=False), name='onboarding_default'),
        path('onboarding/<int:step>/', RedirectView.as_view(url='/', permanent=False), name='onboarding'),
    ]
